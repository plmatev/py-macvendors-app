...
from pymongo.errors import ExecutionTimeout, OperationFailure, NetworkTimeout, ServerSelectionTimeoutError
...
@router.get("/oui/{mac_address}", responses={400: {"content":
                                                        {"application/json":
...
async def get_single_mac_oui(mac_address: str):
    ...
    if not mac_raw:
        raise HTTPException(status_code=400, detail="Invalid or unsupported MAC address format")
    ...
    for db_collection in (
        ...
        except (ExecutionTimeout, OperationFailure, NetworkTimeout, ServerSelectionTimeoutError):
            raise HTTPException(
                status_code=503,
...
@router.get("/vendorname/{mac_address}", responses={400: {"content":
                                                              {"application/json":
                                                                   {"example": {"detail": "string"}}}},
                                                    404: {"content":
                                                              {"application/json":
...
@router.get("/company/{company_name}", responses={404: {"content":
                                                            {"application/json":
                                                                 {"example": {"detail": "string"}}}},
                                                  503: {"content":
                                                            {"application/json":
